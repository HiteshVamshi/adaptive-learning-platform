...
def generate_cbse_math_dataset() -> GeneratedDataset:
    ...
    questions = pd.concat([manual_questions, textbook_questions], ignore_index=True).sort_values(
        by=["chapter_id", "concept_id", "difficulty", "question_id"], ignore_index=True
    )
    solutions = pd.concat([manual_solutions, textbook_solutions], ignore_index=True).sort_values(
        by=["question_id"], ignore_index=True
    )
